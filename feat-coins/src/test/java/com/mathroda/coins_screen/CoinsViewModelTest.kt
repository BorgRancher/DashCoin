...
import com.mathroda.coins_screen.util.MainDispatcherRule
...
class CoinsViewModelTest {
    ...
    fun `when getCoins called and result is Success`() = runTest {
        ...
        every { repository.getCoins(skip) } returns flowOf(Resource.Success(data))
        viewModel.getCoins()
    ...
    fun `when getCoins called and result is Error`() = runTest {
        ...
        every { repository.getCoins(skip) } returns flowOf(Resource.Error(errorMsg))
        viewModel.getCoins()
    ...
    fun `when getCoins called and result is Loading when coins state is empty`() = runTest {
        ...
        every { repository.getCoins(skip) } returns flowOf(Resource.Loading())
        viewModel.getCoins()
